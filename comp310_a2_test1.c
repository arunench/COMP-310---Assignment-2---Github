...
void get_patterns(int expected_result[__TEST_MAX_POD_ENTRY__][__TEST_MAX_KEY__],
    ...
    *pattern_length = 0;
    ...
    for(int k = 0; k < __TEST_MAX_POD_ENTRY__; k++){
        if(patterns[0] == expected_result[k][i]){
            break;
        }else if(patterns[0] > expected_result[k][i]){
            patterns[0] = expected_result[k][i];
            *pattern_length = 1;
        }else if(patterns[*pattern_length - 1] + 1 == expected_result[k][i]){
            patterns[*pattern_length] = expected_result[k][i];
            *pattern_length += 1;
        }else{
...
int main(){
    ...
    for(int i = 0; i < __TEST_MAX_POD_ENTRY__; i++){
        memset(expected_result, 0, __TEST_MAX_KEY__ * sizeof(int));
    }
