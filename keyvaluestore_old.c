...
int kv_store_write(char *key, char *value){
	...
	// char* index_count = strncpy(index_count, addr + index_loc, 3);
	// int index_num = atoi(index_count);
// Truncate key
	// if(strlen(key) > 32){
	...
	while(i < (location_wr + 256*256)){
		...
		if (addr[i] == '\0'){
			...
			// printf("index count: %d", index_num);

			// for(int a = 0; a < 3; a++){
			// 	addr[index_loc + a] = index_loc;
			// }

			// for(int k = 0; k < 3; k++){
			// 	printf("%d", addr[index_loc + k]);
			// }
			
			int offset = i;
			printf("\nAt index %d we insert the characters\n", i);
...
char *kv_store_read(char *key){
	...
	write(STDOUT_FILENO, addr, KV_SIZE);
